...
Double_t TAVTactNtuVertex::ComputeDistance(Double_t zVal)
{
    ...
    for(Int_t q = 0; q<nTracks; ++q ) {
        ...
        meanX += track0->Intersection(zVal).X();
        meanY += track0->Intersection(zVal).Y();
    }
    ...
    for(Int_t qq = 0; qq < nTracks; ++qq) {
        ...
        distance += TMath::Sqrt( (track0->Intersection(zVal).X() - meanX)*(track0->Intersection(zVal).X() - meanX)
								+ (track0->Intersection(zVal).Y() - meanY)*(track0->Intersection(zVal).Y() - meanY) );
    }
...
Double_t TAVTactNtuVertex::ComputeFinalCoordinate(Int_t a, Double_t zVal)
{
    ...
    for(Int_t q = 0; q<nTracks; ++q ){
        TAVTtrack* track0 = ntuTrack->GetTrack(q);
        if(a ==0)
            returnValue += track0->Intersection(zVal).X();
        if(a == 1)
            returnValue += track0->Intersection(zVal).Y();
    }
