/*!
 \file TAGactNtuGlbTrackF.cxx
 \brief   Implementation of TAGactNtuGlbTrackF.
 */
...
Bool_t  TAGactNtuGlbTrackF::fgBmMatched  = false;
Bool_t  TAGactNtuGlbTrackF::fgCaTwUpdate = false;
...
TAGactNtuGlbTrackF::TAGactNtuGlbTrackF(const char* name,
                                       TAGdataDsc* pVtVertex,  TAGdataDsc* pItNtuClus, TAGdataDsc* pMsdNtuClus, TAGdataDsc* pTwNtuRec,
                                       TAGdataDsc* pCaNtuClus, TAGdataDsc* pNtuTrack,  TAGparaDsc* pVtGeoMap,   TAGparaDsc* pItGeoMap,
                                       TAGparaDsc* pMsdGeoMap, TAGparaDsc* pTwGeoMap,  TAGparaDsc* pgGeoMap,    TADIgeoField* field)
:  TAGaction(name, "TAGactNtuGlbTrackF - NTuplize Straight Track"),
   ...
   fEmProperties(new TADItrackEmProperties)
{
   ...
   if (!fpFootGeo)
      Error("TAGactNtuGlbTrackF()", "No GeoTrafo action available yet\n");
   ...
   // track propagator
   fpTrackProp = new TADItrackPropagator(field);
}

//------------------------------------------+-----------------------------------
//! Destructor.
TAGactNtuGlbTrackF::~TAGactNtuGlbTrackF()
{
...
void TAGactNtuGlbTrackF::CreateHistogram()
{
   TString prefix("glb");
...
//! Action
Bool_t TAGactNtuGlbTrackF::Action()
{
   // GLB
...
//_____________________________________________________________________________
//! Check Vertex
Bool_t TAGactNtuGlbTrackF::CheckVtx()
{
   // VTX info
...
//_____________________________________________________________________________
//! Find global tracks
Bool_t TAGactNtuGlbTrackF::FindTracks()
{
...
void TAGactNtuGlbTrackF::FillMcTrackId(TAGcluster* cluster, TAGpoint* point)
{
...
TAGtrack* TAGactNtuGlbTrackF::FillVtxTracks(TAVTtrack* vtTrack)
{
   TAGparGeo* pGeoMapG = (TAGparGeo*)  fpGeoMapG->Object();
   ...
   TAVTcluster* cluster1 = (TAVTcluster*)vtTrack->GetCluster(nClus-1);
   
   TVector3 posG0 = fpFootGeo->FromVTLocalToGlobal(cluster0->GetPositionG());
   TVector3 posG1 = fpFootGeo->FromVTLocalToGlobal(cluster1->GetPositionG());
   ...
   TVector3 momErrG(TMath::Sqrt(errG0[0]*errG0[0]+errG1[0]*errG1[0]), TMath::Sqrt(errG0[1]*errG0[1]+errG1[1]*errG1[1]),
                    TMath::Sqrt(errG0[2]*errG0[2]+errG1[2]*errG1[2]));
   TVector3 tmp(0,0,0);
   ...
   for (Int_t i = 0; i < nClus; ++i) {
      ...
      TAGpoint* point = track->AddPoint(FootBaseName("TAVTparGeo"), posG, errG, tmp, tmp, momG, momErrG);
...
void TAGactNtuGlbTrackF::FindItrCluster(TAGtrack* track)
{
   TAGntuGlbTrack* pNtuTrack = (TAGntuGlbTrack*) fpNtuTrack->Object();
...
//!
//! \param[in] track a given track
void TAGactNtuGlbTrackF::FindMsdCluster(TAGtrack* track)
{
   TAGntuGlbTrack*  pNtuTrack = (TAGntuGlbTrack*)  fpNtuTrack->Object();
...
//!
//! \param[in] track a given track
void TAGactNtuGlbTrackF::FindCaCluster(TAGtrack* track)
{
   TAGntuGlbTrack* pNtuTrack = (TAGntuGlbTrack*) fpNtuTrack->Object();
...
//!
//! \param[in] track a given track
void TAGactNtuGlbTrackF::ComputeMass(TAGtrack* track)
{
...
//!
//! \param[in] track a given track
void TAGactNtuGlbTrackF::FillHistogramm(TAGtrack* track)
{
...
//_____________________________________________________________________________
//! Fill histograms
void TAGactNtuGlbTrackF::FillHistogramm()
{
   TAGntuGlbTrack* pNtuTrack = (TAGntuGlbTrack*) fpNtuTrack->Object();
...
//!
//! \param[in] track a given track
void TAGactNtuGlbTrackF::UpdateParam(TAGtrack* track)
{
...
vector<double> TAGactNtuGlbTrackF::GetLinearFit(const vector<double>& z, const vector<double>& x, const vector<double>& dx)
{
