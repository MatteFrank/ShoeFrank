...
#include "TAPWdigitizer.hxx"

// Compute the number of photons for 5 mm slice width

// --------------------------------------------------------------------------------------
TAPWdigitizer::TAPWdigitizer(TAPWntuHit* pNtuRaw)
 : TAGobject(),
   fpNtuRaw(pNtuRaw),
   fGain(5.e-4),
   fEnergyResCst(-0.004),
...
// --------------------------------------------------------------------------------------
void  TAPWdigitizer::SetFunctions()
{
   // compute birks law for a given Edep
   fFuncEnergyRes = new TF1("PhotonsTR", this, &TAPWdigitizer::RecEnergyRes, 0, 20, 3, "TAPWdigitizer", "RecEnergyRes");
}

// --------------------------------------------------------------------------------------
void  TAPWdigitizer::SetParFunction()
{
   //all in mm
...
// --------------------------------------------------------------------------------------
TAPWdigitizer::~TAPWdigitizer()
{   
   delete fFuncEnergyRes;
}

// --------------------------------------------------------------------------------------
Double_t TAPWdigitizer::RecEnergyRes(Double_t* x, Double_t* par)
{
   Float_t en  = x[0];
...
Float_t TAPWdigitizer::GetEnergyRes(Float_t /*X*/, Float_t /*Y*/, Float_t edep)
{
   return fFuncEnergyRes->Eval(edep);
}

//___________________________________________________________________________________________
Bool_t TAPWdigitizer::Process(Double_t edep, Double_t x0, Double_t y0, Double_t /*zin*/, Double_t /*zout*/, Double_t time, Int_t id, Int_t /*Z*/, Double_t /*px0*/, Double_t /*py0*/, Double_t /*pz0*/)
{
   ...
   if (fMap[id] == 0) {
     fCurrentHit = (TAPWhit*)fpNtuRaw->NewHit();
