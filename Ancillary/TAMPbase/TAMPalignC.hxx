#ifndef _TAMPalignC_HXX
#define _TAMPalignC_HXX
/*!
 \file TAMPalignC.cxx
 \brief   Declaration of TAMPalignC.
...
class TObjArray;
class TAMPalignC : public TObject {
   
private:
   static TAMPalignC* fgInstance; ///< static instance of class
   
public:
   // Instance of class
   static TAMPalignC* Instance(const TString name = "16O_C2H4_200_1.root", const TString exp = "", Int_t runNUmber = 1,
                               Int_t weight = -1); // The weight is barely influencing the results
   virtual ~TAMPalignC();
   
private:
   // ctr
   TAMPalignC(const TString name, const TString exp, Int_t runNUmber, Int_t weight);
public:
   // Loop over events
...
private:
   ...
   void    FillPosition();
   // Fill status map
   void    FillStatus(TAVTbaseParConf* parConf);
   // Fill position map
   void    FillPosition(TAVTbaseParGeo* parGeo);
