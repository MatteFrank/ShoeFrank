...
Bool_t TAMPparConf::FromFile(const TString& name)
{   
   ...
   for (Int_t p = 0; p < fSensorsN; p++) { // Loop on each plane
     ...
     // read min of charge per cluster
     ReadItem(fSensorParameter[p].MinNofChargesInPixel);
      if(FootDebugLevel(1))
        cout << "Minimum number of charge per pixel: "<< fSensorParameter[p].MinNofChargesInPixel << endl;
...
Int_t TAMPparConf::GetStatus(Int_t idx) const
{
...
//!
//! \param[in] idx status number
Int_t TAMPparConf::GetStatusMinor(Int_t idx) const
{
   Int_t major = fSensorParameter[idx].Status/10;
   if (major == 0) {
...
//------------------------------------------+-----------------------------------
//! Clear geometry info.
void TAMPparConf::Clear(Option_t*)
{
  return;
}
...
Bool_t TAMPparConf::IsDeadPixel(Int_t sensorId, Int_t aLine, Int_t aColumn)
{
...
void TAMPparConf::ToStream(ostream& os, Option_t*) const
{
//  os << "TAMPparConf " << GetName() << endl;
