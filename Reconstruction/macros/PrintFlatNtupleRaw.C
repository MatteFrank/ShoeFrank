...
// main
void PrintFlatNtupleRaw_new(TString expName = "12C_200", Int_t runNumber = 1, TString nameFile = "", Int_t nentries = 0)

{
  ...
  tree->SetBranchAddress(TAMSDntuPoint::GetBranchName(), &msPoint);
  ...
  tree->SetBranchAddress(TATWntuPoint::GetBranchName(), &twpoint);
  ...
  if ((IncludeMC))
  {
    ...
    if ((IncludeMC))
    {
      ...
      if (IncludeMC)
      {
        <Int_t> vector<Int_t> TW_McID_Hit;
      }
      ...
      if (IncludeMC)
      {
        fClusMsd_MCId; ///< MSD point number
      }
        ...
        for (Int_t ev = 0; ev < nentries; ++ev)
        {
          ...
          for (Int_t iPlane = 0; iPlane < nPlanes; iPlane++)
          {
            ...
            for (Int_t iClus = 0; iClus < nclus; ++iClus)
            {
              ...
              if ((IncludeMC))
              {
                TAMCpart *track = eve->GetTrack(clus->GetMcTrackIdx(0));
