...
void TAMSDactNtuMC::FillStrips(Int_t sensorId, Int_t hitId )
{
  ...
  // float position;
  // if ( view == 0 ) { //X
  //   position = fpEvtStr->MSDxin[hitId];
  // }
  // else { //Y
  //   position = fpEvtStr->MSDyin[hitId];
  // }
  //
  // //tenere conto del layer
	// int stripId = pGeoMap->GetStrip( view, position );
  //
	// TAMSDntuHit* strip = (TAMSDntuHit*)pNtuRaw->NewStrip(sensorId, 1., view, stripId);
