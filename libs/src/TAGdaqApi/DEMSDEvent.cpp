...
void DEMSDEvent::fillStrip(){
  ...
  if(evtSize!=0){
	...
	if (ADC < 2){
	  //	  Xplane[ADC*256+k] = (data & 0xFFFF)/4;
	  //	  Xplane[ADC*256+128+k] = ((data >> 16) & 0xFFFF)/4;
	  Yplane[ADC*256+k] = ((data >> 16) & 0xFFFF)/4;
	  Yplane[ADC*256+128+k] = (data & 0xFFFF)/4;
	}
	else if (ADC == 2){
	  //Xplane[ADC*256+k] = (data & 0xFFFF)/4;
	  //Yplane[(ADC-2)*256+k] = ((data >> 16) & 0xFFFF)/4;
	  Yplane[ADC*256+k] = ((data >> 16) & 0xFFFF)/4;
	  Xplane[(ADC-2)*256+k] = (data & 0xFFFF)/4;
	}
	else if(ADC == 3){
	  //	  Yplane[128+k] = (data & 0xFFFF)/4;
	  //	  Yplane[256+k] = ((data >> 16) & 0xFFFF)/4;
	  Xplane[128+k] = ((data >> 16) & 0xFFFF)/4;
	  Xplane[256+k] = (data & 0xFFFF)/4;
	}
	else if(ADC==4){
	  //	  Yplane[384+k] = (data & 0xFFFF)/4;
	  //	  Yplane[512+k] = ((data >> 16) & 0xFFFF)/4;
