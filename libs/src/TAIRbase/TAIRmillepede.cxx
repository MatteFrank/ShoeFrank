							...
							double lResCut, double lResCutInit)
{
   ...
   TAIRmillepede::SetNLocalEquations(0);     // Number of local fits (starts at 0)
   ...
   for (int j=0; j<fNGlobalPar; j++) {
	  TAIRmillepede::SetParSigma(j,0.0);
   }
   ...
   fLocEqPlace.Reset();  fNLocEqPlace=0;
    ...
    if(FootDebugLevel(1)) {
	  ...
	  printf("----------------------------------------------------\n");
	  printf("\n");
	  printf("    InitMille has been successfully called!\n");
	  printf("\n");
	  printf("-----------------------------------------------------\n");
	  printf("\n");
   }
...
Int_t TAIRmillepede::SetIterations(double lChi2CutFac)
{
   ...
   printf("Initial cut factor is %f\n",fChi2CutFactor);
...
{ 
   ...
   fNGlobalConstraints++ ;
   printf("Number of constraints increased to %d\n",fNGlobalConstraints);
   return 1;
}
...
{	
   ...
   FootDebugLine(2, "SetLocalEquation()", Form("%d / %d",iLocFirst, iLocLast));
   ...
   FootDebugLine(2, "SetLocalEquation()", Form("%d / %d",iGlobFirst,iGlobLast));
   ...
   FootDebugLine(2, "SetLocalEquation()", Form("Out Equloc --  NST = %d",fNDerivLocEq));
   return 1; 	
}
...
Int_t TAIRmillepede::LocalFit(int iFit, double localParams[], Bool_t bSingleFit)
{
   ...
   if (fIter < 2 && !bSingleFit) { // Do it only once 
      ...
      FootDebugLine(2, "LocalFit()", Form("FLocEqPlace size = %d",fLocEqPlace[iFit]));
      FootDebugLine(2, "LocalFit()", Form("FIndexAllEqs size   = %d",fNIndexAllEqs));
   }
   ...
   while (iEqTerm <= nEqTerms) {
	  if (fIndexLocEq[iEqTerm] == -1) {
         ...
         if(FootDebugLevel(1)) {
            ...
            FootDebugLine(2, "LocalFit()", Form("fDeltaPar[%d] = %f", iIdx, fDeltaPar[iIdx]));
            FootDebugLine(2, "LocalFit()", Form("Starting misalignment = %f",fInitPar[iIdx]));
          ...
          FootDebugLine(2, "LocalFit()", Form("lMeas after global stuff removal = %f", lMeas));
            ...
            FootDebugLine(2, "LocalFit()", Form("fVecBLoc[%d] = %f", iIdx, fVecBLoc[iIdx]));
			   
			   for (j=iLocFirst; j<=i ; j++) { // Symmetric matrix, don't bother j>i coeffs
				  jIdx = fIndexLocEq[j];						
				  fMatCLoc[iIdx][jIdx] += lWeight*fDerivLocEq[i]*fDerivLocEq[j];	    
               FootDebugLine(2, "LocalFit()", Form("fMatCLoc[%d][%d] = %g", iIdx, jIdx, fMatCLoc[iIdx][jIdx]));
			   }
			}
			iMeas   = -1;
			iWeight = 0;
   ...
   // nRank is the number of nonzero diagonal elements 
    ...
    if(FootDebugLevel(1)) {
	  ...
	  printf(" Result of local fit :      (index/parameter/error)\n");
	  
	  for (i=0; i<fNLocalPar; i++) {
		 printf("%d   /   %.6f   /   %.6f\n", i, fVecBLoc[i], TMath::Sqrt(fMatCLoc[i][i]));	
	  }
   ...
   while (iEqTerm <= nEqTerms) {
	  if (fIndexLocEq[iEqTerm] == -1) {
		 if (iMeas == -1) {        // First  -1 : lMeas
			...
			// 	FootDebugLine(2,"LocalFit()", Form(". equation:  measured value %.6f +/- %.6f", lMeas, 1.0/TMath::Sqrt(lWeight)));
			// 	FootDebugLine(2,"LocalFit()", Form("Number of derivatives (global, local): %d, %d",nDerGlo,nDerLoc));
			// 	FootDebugLine(2,"LocalFit()", "Global derivatives are: (index/derivative/parvalue) ");
			
			// 	for (i=iGloFirst; i<=iGloLast; i++) {
			// 	  FootDebugLine(2,"LocalFit()", Form("%d / %.6f / %.6f",fIndexLocEq[i],fDerivLocEq[i],fInitPar[fIndexLocEq[i]]));
			//      } 
			
			// 	FootDebugLine(2, "LocalFit()", "Local derivatives are: (index/derivative) ");
			
			// 	for (i=(ja+1); i<jb; i++) {FootDebugLine(2, "LocalFit()", "LocalFit()", Form("%d / %.6f",fIndexLocEq[i], fDerivLocEq[i]));}
          ...
          FootDebugLine(2,"LocalFit()", Form("Residual value : %.6f", lMeas));
			
			// reject the track if lMeas is too important (outlier)
			if (TMath::Abs(lMeas) >= fResCutInit && fIter <= 1) {
            FootDebugLine(2, "LocalFit()", "Rejected track !!!!!");
			   fNLocalFitsRejected++;      
   ...
   while (iEqTerm <= nEqTerms)
   {
	  if (fIndexLocEq[iEqTerm] == -1)
	  {
		 if (iMeas == -1) {        // First  -1 : lMeas
            ...
            FootDebugLine(2, "LocalFit()", Form("fVecBGlo[%d] = %.6f", j, fVecBGlo[j] ));
               ...
               FootDebugLine(2, "LocalFit()", Form("fMatCGlo[%d][%d] = %.6f",iIdx,jIdx,fMatCGlo[iIdx][jIdx]));
			   } 
			   ...
			   for (k=iLocFirst; k<=iLocLast ; k++) {
				  ...
				  FootDebugLine(2, "LocalFit()", Form("fMatCGloLoc[%d][%d] = %.6f",iIdxIdx,kIdx,fMatCGloLoc[iIdxIdx][kIdx]));
			   } 
			}
			iMeas   = -1;
			iWeight =  0;
   ...
   TAIRmillepede::SpAVAt(fMatCLoc, fMatCGloLoc, fMatCGloCorr, fNLocalPar, nGloInFit);
   TAIRmillepede::SpAX(fMatCGloLoc, fVecBLoc, fVecBGloCorr, fNLocalPar, nGloInFit);
...
Int_t TAIRmillepede::GlobalFit(double par[], double error[], double pull[])
{
   ...
   printf("\n..... Making global fit .....\n");
   printf("First Iteration with cut factor %.2f\n", fChi2CutFactor);
   
   nLocFitsTot = TAIRmillepede::GetNLocalEquations();
   
   while (fIter < fMaxIter)  // Iteration for the final loop
   {
	  
	  nLocFits = TAIRmillepede::GetNLocalEquations();
	  printf("...using %d local fits...\n",nLocFits);
	  ...
	  printf("Number of constraint equations : %d\n", fNGlobalConstraints);
	  ...
	  printf(" Final coeff is %.6f\n",lFinalCor);		
	  printf(" Final NDOFs = %d\n", fNGlobalPar);
	  
	  //  The final matrix inversion
	  
	  Int_t nRank = TAIRmillepede::SpmInv(fMatCGlo, fVecBGlo, nVar);
	  ...
	  printf("The rank defect of the symmetric %d by %d matrix is %d (bad if non 0)\n",
			 nVar, nVar, nVar-nGloFix-nRank);
	  
	  printf("Total : %d local fits, %d rejected.\n", fNLocalFits, fNLocalFitsRejected);
	  ...
	  printf("\n");
	  printf("----------------------------------------------\n");
	  printf("Iteration %d with cut factor %.2f\n", fIter, fChi2CutFactor);
	  ...
	  for (i=0; i<nLocFitsTot; i++) {
        ...
        FootDebugLine(2, "GlobalFit()", Form("Track %d : ",i));
        FootDebugLine(2, "GlobalFit()", Form("Starts at %d", iEqFirst));
        FootDebugLine(2, "GlobalFit()", Form("Ends at %d",iEqLast));
		 ...
		 if (fIndexAllEqs[iEqFirst] != -999) { // Fit is still OK      
			...
			//  	Int_t sc = TAIRmillepede::LocalFit(i,localPars,0);
			// 	(sc) ? nLocFitsGood++ : fIndexAllEqs[iEqFirst] = -999;   
			TAIRmillepede::LocalFit(i,localPars,0);
			nLocFitsGood++;
		 }
	  } // End of loop on fits
	  
	  TAIRmillepede::SetNLocalEquations(nLocFitsGood);
	  
   } // End of iteration loop
   
   TAIRmillepede::PrintGlobalParameters(); // Print the final results
...
/*
 ...
 -----------------------------------------------------------
 */
Int_t TAIRmillepede::PrintGlobalParameters() const
{
   ///  Print the final results into the logfile
   double lError = 0.;
   double lGlobalCor =0.;
   ...
   printf("   ===================================\n");
   printf("    I       initial       final       differ        lastcor        error       gcor\n");
   printf("-----------------------------------------------------------------------------------\n");
   ...
   for (int i=0; i<fNGlobalPar; i++) {
	  ...
	  if (TMath::Abs(fMatCGlo[i][i]*fDiagCGlo[i]) > 0) {    
		 ...
		 printf("%d\t %.6f\t %.6f\t %.6f\t %.6f\t OFF\t OFF\n",i,fInitPar[i],fInitPar[i]+fDeltaPar[i],fDeltaPar[i],fVecBGlo[i]);
	  }
   }
   return 1;
}
