...
public:
    ...
    void Action() override {
        ...
        for(auto & hypothesis : hypothesis_c){
            ...
            std::cout << "track_slope_x = " << particle_m.track_slope_x << '\n';
            std::cout << "track_slope_y = " << particle_m.track_slope_y << '\n';
...
private:
    ...
    std::vector<particle_properties> form_hypothesis()
    {
        ...
        for( const auto& candidate : candidate_c ) {
            ...
            auto track_slope_y = ( endPosition.Y() - vertexPosition.Y() )/( endPosition.Z() - vertexPosition.Z() );
            
            hypothesis_c.push_back(
                particle_properties{ charge, mass, momentum, track_slope_x, track_slope_y }
            );
        }
    ...
    void reconstruct()
    {
        ...
        std::cout << "candidate_c::size : " << candidate_c.size() << '\n';
                                ...
                                corrected_state{
                                    state{
                                        ...
                                        make_state_vector( vector{std::move(candidate_p.vector)},
                                                       vector_matrix{{  particle_m.track_slope_x, particle_m.track_slope_y }} ),
        ...
        std::cout << "root_c::size : " << root_c.size() << '\n';
...
//
            ...
            std::cout << "propagated_state : (" <<  ps.vector(0,0) << ", " << ps.vector(1,0) << ", " << ps.evaluation_point << ")\n";
            ...
            std::for_each( candidate_c.begin(), candidate_c.end(),
                            [this, &ps, &enriched_c]( const auto& candidate_p )
                            {
                                ...
                                std::cout << "candidate : (" <<  candidate_p.vector(0,0) << ", " << candidate_p.vector(1,0) << ")\n";
...
//                                      );
            ...
            for(auto iterator = enriched_c.begin() ; iterator != end ; ++iterator ){
                ...
                std::cout << "corrected_state : (" <<  cs.vector(0,0) << ", " << cs.vector(1,0) << ", " << cs.evaluation_point << ")\n";
